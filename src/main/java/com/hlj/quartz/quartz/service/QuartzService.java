...
import com.hlj.quartz.quartz.Job.HelloJobThree;
import org.quartz.*;
...
public class QuartzService {
    ...
    public void secondOnce() throws SchedulerException {
        JobDetail jobDetail = JobBuilder.newJob(HelloJobThree.class).withIdentity("job3", "group3").build();
        SimpleScheduleBuilder simpleScheduleBuilder = SimpleScheduleBuilder.simpleSchedule().withIntervalInSeconds(5).withRepeatCount(0);
        Trigger trigger = TriggerBuilder.newTrigger().withIdentity("job3", "group3").withSchedule(simpleScheduleBuilder).startAt(new Date(System.currentTimeMillis() + 5000L)).build();
        System.out.println("三号任务启动" + new Date());
        scheduler.scheduleJob(jobDetail, trigger);
    }
}
